...
app.post('/addproduct',async (req,res)=>{
    ...
    if(products.length > 0){
        ...
        let last_array = last_product_array[0];
        id = last_array.id+1;
    }
    else{
        id = 1;
...
//Creating Endpoints for regestring the user
...
//ceating endpoins for new collection data

app.get('/newcollections', async (req,res)=>{
    let products = await Product.find({});
    let newcollection = products.slice(1).slice(-8);
    console.log('New Collection Fethched')
...
//creating endpoints to get the cart data
...
app.listen(port,(error)=>{
    if(!error){
        console.log(`Server listen from port number ${port}`);
    }
    else{
